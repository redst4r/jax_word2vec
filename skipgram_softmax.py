...
class SkipgramSM():
    ...
    Explicitly working on batches of data
    """
    def __init__(self):
        pass

    @partial(jax.jit, static_argnums=(0,))
    def _forward(self, params,  target):
    ...
    @partial(jax.jit, static_argnums=(0,))
    def loss(self, params, target, context):
        """Compute the loss of the word2Vec model."""
        V, D = params['projection'].shape
    
        logits = self._forward(params, target)  # batch, vocab
...
class SkipgramSM_single():
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _loss_single_sample(self, params, target, context):
        """Compute the loss of the word2Vec model."""
        
        V, D = params['projection'].shape
        logits = self._forward(params, target)  #vocab,
    ...
    @partial(jax.jit, static_argnums=(0,))
    def loss(self, params, target_batch, context_batch):
        loss_per_sample = jax.vmap(self._loss_single_sample, in_axes=[None,0,0])
